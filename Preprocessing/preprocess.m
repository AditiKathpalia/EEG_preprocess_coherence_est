...
function [EEG_clean] = preprocess(EEG,cap,labels)  
...
EEG = pop_select(EEG, 'nochannel', chans_to_remove);    %EEG = pop_select(EEG, 'channel', chan_to_retain);    
EEG = eeg_checkset(EEG);
...
% Remove first and last 30s of data
pts_to_remove=30*EEG.srate;
EEG = pop_select( EEG,'nopoint',[1 pts_to_remove; EEG.pnts-pts_to_remove+1 EEG.pnts] );
